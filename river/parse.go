...
func (r *River) makeInsertRequest(e *canal.RowsEvent) ([]*DmlMsg, error)  {
	...
	for _, row := range e.Rows {
		...
		appendRow1 := ""//make(map[int]string, 1)
		//appendRow1[0] =
		appendRow2 := "" //make(map[int]string, 1)
		//appendRow2[0]  = rule.Tschema
		row = append(row,appendRow1,appendRow2)
		...
		for ii, v1 := range row {
			...
			for _, v := range PKColumns {
				if v == ii {
					keybuf.WriteString(str)
					appendRow1,err = getStrByColType(e.Table.Columns[0].Type, row[0])
					break
				}
			}
			if err != nil{
			...
			if e.Table.Columns[ii].Name == "groupID" {
				appendRow2, err = getStrByColType(e.Table.Columns[ii].Type, v1)
			}
			if e.Table.Columns[ii].Name == "source_db_name" {
				str = rule.Tschema
			}
			if e.Table.Columns[ii].Name == "id" {
				str = strings.Join([]string{appendRow1, appendRow2}, "-")
			}
		...
		fmt.Sprintf("%s (dmlMsgArr)", dmlMsgArr)
	}
	log.Debug("dmlMsgArr  %s", dmlMsgArr)
...
func (r *River) getUpdateSql(head string, oldIndex int, newIndex int, setIndexs map[int]int, indexs map[int]int,
	...
	oldRows := e.Rows[oldIndex]

	newRows := e.Rows[newIndex]
	...
	for i, col := range colnames{
		if len(indexs) == 0 {
			...
			oldRows = append(e.Rows[oldIndex],appendOldRow1,appendOldRow2)

			oldvalue, err := getStrByColType(col.Type, oldRows[i])
			if err != nil{
				return nil, err
			}

			if col.Name == "groupID"{
				concatStr, _ = getStrByColType(e.Table.Columns[i].Type, oldRows[i])
			}


			appendNewRow1 := ""
			appendNewRow2 := ""
			newRows = append(e.Rows[newIndex],appendNewRow1,appendNewRow2)

			newvalue, err := getStrByColType(col.Type, newRows[i])
			if err != nil{
				return nil, err
			}
			...
			if _, ok := indexs[i]; ok{
				...
				oldvalue, _ := getStrByColType(e.Table.Columns[0].Type, oldRows[0])
				//concat primary conditon
				oldvalue = strings.Join([]string{oldvalue, concatStr}, "-")
...
func (r *River) GetDelSql(head string, rule *Rule, e *canal.RowsEvent) ([]*DmlMsg, error) {
	...
	if len(indexs) == 0 {
		...
		for _, row := range e.Rows {
			...
			appendRow := make([]interface{}, 2);
			appendRow[0] = ""
			appendRow[1]  = rule.Tschema
			row =append(row,appendRow);
		...
		for _, row := range e.Rows {
			...
			for ii, index := range indexs {
				str, err := getStrByColType(e.Table.Columns[0].Type, row[0])
				if err != nil{
					return nil, err
				}
				for iv, v := range e.Table.Columns {
					  if v.Name == "groupID"{
						  appendRow2, err = getStrByColType(e.Table.Columns[iv].Type, row[iv])
						  break
					  }
				}
				//concat primary key
				str = strings.Join([]string{str, appendRow2}, "-")
				keybuf.WriteString(str)
				if err != nil {
					return nil, err
				}
